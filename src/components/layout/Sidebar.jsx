...
function SidebarHorizontal({ menuItems, theme, isGlass }) {
  ...
  useEffect(() => {
    ...
    const fn = (e) => {
      if (!e.target.closest("[data-navitem]") && !e.target.closest(".sv-dropdown")) setOpenIdx(null);
    };
  ...
  const toggleAH  = () => { const next=!autoHide; setAutoHideState(next); setAutoHideLS(next); setVisible(!next); };
  const scrollNav = (dir) => { scrollRef.current?.scrollBy({ left:dir*160, behavior:"smooth" }); setTimeout(checkScroll, 350); };
  ...
  return (
    <>
      {autoHide && !visible && (
        <div style={{ position:"fixed", top:0, left:0, right:0, height:8, zIndex:201, pointerEvents:"all" }} onMouseEnter={showBar}/>
      )}
      <div onMouseEnter={showBar} onMouseLeave={startHide} style={{
        position:"fixed", top:0, left:0, right:0, zIndex:200, height:54,
        background:"rgba(12,16,32,0.90)", backdropFilter:"blur(28px) saturate(180%)",
        WebkitBackdropFilter:"blur(28px) saturate(180%)",
            ...
            {menuItems.map((item, idx) => {
              ...
              return (
                <div key={item.to} style={{ position:"relative", flexShrink:0 }}>
                  <div data-navitem={idx} onClick={e=>handleItemClick(item,idx,e.currentTarget)} style={{
                    display:"flex",alignItems:"center",gap:6,padding:"6px 11px",borderRadius:8,cursor:"pointer",
                    whiteSpace:"nowrap",userSelect:"none",color:active?"#fff":"rgba(255,255,255,0.65)",
                    background:active?theme.primary:"transparent",fontWeight:active?600:400,fontSize:13,transition:"all 0.18s",
                  }}
        ...
        <div style={{ display:"flex",alignItems:"center",gap:6,padding:"0 12px",flexShrink:0 }}>
          <button onClick={toggleAH} title={autoHide?"Fixar":"Auto-hide"} style={{ background:"transparent",border:"none",color:autoHide?theme.primary:"rgba(255,255,255,0.4)",fontSize:16,cursor:"pointer",padding:"4px 6px",borderRadius:6 }}>
            {autoHide?"📌":"👁"}
          </button>
...
// ═══════════════════════════════════════════════════════════════
// ESTILO 3/4 — DOCK CONVEXO/CÔNCAVO (Desktop + Mobile)
// ═══════════════════════════════════════════════════════════════
function SidebarDock({ menuItems, theme, isGlass, convex = true, isMobileMode = false }) {
  const location = useLocation();
  const isActive = p => location.pathname === p;
  const navigate = useNavigate();
  ...
  const HAM_KEY   = `sv_dock_ham_${convex?"conv":"conc"}${isMobileMode?"_m":""}`;
  ...
  const getInitialPos = useCallback(() => {
    ...
    return {
      x: convex ? (isMobileMode ? 14 : 14) : (ww - 56),
      y: Math.round(vh / 2 - 21),
    };
  }, [HAM_KEY, convex, isMobileMode, ww, vh]);

  const [hamPos, setHamPos] = useState(getInitialPos);
  const hamSize = isMobileMode ? 48 : 42;
  ...
  const moveDrag = useCallback((clientX, clientY) => {
    ...
    setHamPos({
      x: Math.max(0, Math.min(ww - hamSize, dragStart.current.bx + dx)),
      y: Math.max(0, Math.min(vh - hamSize, dragStart.current.by + dy)),
    });
  }, [ww, vh, hamSize]);
  ...
  const R       = isMobileMode ? 28 : 26;
  const GAP     = isMobileMode ? 12 : 10;
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          background:open ? theme.primary : isGlass ? "rgba(255,255,255,0.22)" : "rgba(20,26,50,0.95)",
          border:`2px solid ${open ? theme.primary : isGlass ? "rgba(255,255,255,0.4)" : "rgba(255,255,255,0.2)"}`,
          backdropFilter:"blur(16px)", WebkitBackdropFilter:"blur(16px)",
          boxShadow:open
            ? `0 0 0 6px ${theme.primary}30, 0 10px 36px ${theme.primary}55`
        ...
        {/* Hamburger lines → X */}
        {[
          { mb:open?0:5, rotate:open?"rotate(45deg) translate(0,3px)":"none" },
          { mb:open?0:5, opacity:open?0:1 },
          { mb:0, rotate:open?"rotate(-45deg) translate(0,-3px)":"none" },
        ].map((s, i) => (
          <span key={i} style={{
            display:"block", width:isMobileMode?20:18, height:2, borderRadius:2,
            background:"rgba(255,255,255,0.9)",
            marginBottom:s.mb, opacity:s.opacity??1,
            transform:s.rotate??"none", transition:"all 0.28s ease",
          }}/>
        ))}
      </div>
      ...
      {allItems.map((item, i) => {
        ...
        return (
          <div key={`${item.to}-${i}`} style={{
            position:"fixed", left:bubbleX, top:cy - R,
            width:R * 2, height:R * 2, zIndex:400,
            ...
            <div style={{
              ...
              border:`2.5px solid ${
                ...
                isGlass       ? "rgba(255,255,255,0.45)" :
                                "rgba(255,255,255,0.2)"
              }`,
              display:"flex", alignItems:"center", justifyContent:"center",
              fontSize:isMobileMode ? 20 : 17, cursor:"pointer",
              transform:`scale(${hov ? 1.28 : active ? 1.12 : 1}) translateX(${pushX}px)`,
              ...
              {isLog
                ...
                : <Link to={item.to} style={{ textDecoration:"none", display:"flex", alignItems:"center", justifyContent:"center", width:"100%", height:"100%", borderRadius:"50%" }}>
                    <span style={{ fontSize:isMobileMode?20:17 }}>{item.icon}</span>
                  </Link>
              }
            </div>
            ...
            {hov && (
              <div style={{
                position:"absolute",
                left: isRight ? "auto" : R*2+10,
                right:isRight ? R*2+10 : "auto",
                top:"50%", transform:"translateY(-50%)",
                background:"rgba(8,12,28,0.97)",
                color:isLog ? "#f87171" : "rgba(255,255,255,0.95)",
                padding:"5px 13px", borderRadius:9, fontSize:isMobileMode?14:13,
...
// ═══════════════════════════════════════════════════════════════
// MOBILE — com seletor de estilo incluindo Dock
// ═══════════════════════════════════════════════════════════════
const MOBILE_STYLES = [
  { id:"vertical",   icon:"▐", label:"Lateral",     desc:"Desliza da esquerda" },
  { id:"right",      icon:"▌", label:"Direita",      desc:"Desliza da direita"  },
...
function StylePicker({ styles, current, onSelect, theme, isGlass, border, inline }) {
  const base = {
    padding:inline?"8px 6px":"10px 8px", borderRadius:inline?8:10, cursor:"pointer",
    transition:"all 0.15s", textAlign:"center",
    fontSize:inline?11:12, fontWeight:"inherit",
  };
  const grid = <div style={{ display:"grid", gridTemplateColumns:inline?"repeat(3,1fr)":"1fr 1fr", gap:inline?6:8 }}>
    {styles.map(s => (
  ...
  return (
    <div style={{
      position:"fixed", top:58, left:14, zIndex:600,
...
function SidebarMobile({ menuItems, theme, isGlass }) {
  ...
  // Dock mobile — usa o mesmo componente do desktop
  if (mStyle === "dock") {
    return (
      <>
        <SidebarDock menuItems={menuItems} theme={theme} isGlass={isGlass} convex={true} isMobileMode={true}/>
        {/* Botão de trocar estilo no canto */}
        <button onClick={() => setShowStyles(s => !s)} style={{
          ...
          color:theme.textPrimary, fontSize:14, width:38, height:38,
          cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center",
        }}>⚙</button>
        {showStyles && (
          <StylePicker styles={MOBILE_STYLES} current={mStyle} onSelect={setMStyle} theme={theme} isGlass={isGlass} border={border}/>
        )}
      </>
    );
  }
    ...
    return (
      <>
        <div style={{
          ...
          <button onClick={() => setShowStyles(s => !s)} style={{ background:"transparent", border:"none", color:theme.textMuted, fontSize:14, cursor:"pointer", flexShrink:0, padding:"4px 6px", borderRadius:6 }}>⚙</button>
          {menuItems.map(item => (
        ...
        {showStyles && <StylePicker styles={MOBILE_STYLES} current={mStyle} onSelect={setMStyle} theme={theme} isGlass={isGlass} border={border}/>}
      </>
    );
  }

  // Painéis deslizantes (vertical, right, bottom)
  const panelStyle = () => {
    const base = {
      position:"fixed", zIndex:160, background:bg,
      backdropFilter:backdrop, WebkitBackdropFilter:backdrop,
  ...
  return (
    <>
      <button onClick={() => setOpen(!open)} style={hamStyle()}>{open ? "✕" : "☰"}</button>
      ...
      <div style={panelStyle()}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"18px 18px 14px", borderBottom:`1px solid ${border}`, flexShrink:0 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={() => setShowStyles(s => !s)} style={{ background:showStyles?`${theme.primary}22`:"transparent", border:`1px solid ${showStyles?theme.primary:border}`, color:showStyles?theme.primary:theme.textMuted, borderRadius:8, width:32, height:32, cursor:"pointer", fontSize:13 }}>⚙</button>
            <button onClick={() => setOpen(false)} style={{ background:`${theme.primary}22`, border:"none", color:theme.textPrimary, borderRadius:8, width:32, height:32, cursor:"pointer", fontSize:14 }}>✕</button>
          </div>
        </div>
        ...
        <div style={{ padding:12, flexShrink:0 }}>
          <button onClick={() => { logoutUser(); navigate("/"); }} style={{ width:"100%", padding:"13px 16px", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.25)", borderRadius:12, color:"#ef4444", fontSize:15, fontWeight:600, cursor:"pointer", textAlign:"left" }}>
            🚪 Sair
          </button>
        </div>
      </div>
...
export default function Sidebar({ sidebarOpen, setSidebarOpen }) {
  ...
  const isMobile  = useIsMobile(); // ← hook reativo, corrige o bug principal
